<div class="max-w-3xl mx-auto">
    {{-- Header --}}
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __('campaigns.edit_campaign') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ $campaign->name }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
        <form wire:submit="update">
            ...
            <div class="flex items-center justify-between">
                <flux:button href="{{ route('campaigns.show', $campaign) }}" variant="ghost">
                    {{ __('contacts.cancel') }}
                </flux:button>
                <div class="flex gap-3">
                    <flux:button type="submit" variant="primary" icon="check">
                        {{ __('campaigns.update_campaign') }}
                    </flux:button>
                </div>
            </div>
        </form>
